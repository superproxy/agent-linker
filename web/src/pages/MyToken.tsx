...
export function MyTokenPage(props: {
  ...
  onAuthError: AuthErrorHandler;
  isAdmin: boolean;
  /** false：本机 local 或仅网关 token 模式，不展示 pat_ 自助（API 也不支持） */
  showPat?: boolean;
}) {
  const showPat = props.showPat !== false;
  ...
  if (props.isAdmin) {
    tabItems.push({
      key: 'ct',
      label: '微信终端（ct_）',
      children: <ChannelTerminalKeyPanel base={props.base} token={props.token} onAuthError={props.onAuthError} />,
    });
  }
  ...
  return (
    <div>
      <p className="page-desc" style={{ marginBottom: 12 }}>
        <strong>用户 Key</strong>：登录账号的 <strong>pat_</strong>；管理员可管理微信对话对象的 <strong>ct_</strong>。
      </p>
      {!showPat && !props.isAdmin ? (
        <Alert
          type="info"
          showIcon
          message="当前凭据无法在此页管理 Key"
          description="本机 local 模式请使用「本机 · 网关」中的静态 token；pat_ 需使用账号密码登录后再打开本页。"
        />
      ) : null}
      {!showPat && props.isAdmin && tabItems.length === 1 ? (
        <Alert
          type="info"
          showIcon
          style={{ marginBottom: 12 }}
          message="当前为网关静态 token 模式"
          description="pat_ 需账号密码登录后签发；下方为微信终端 ct_ 管理。"
        />
      ) : null}
      {tabItems.length === 1 ? tabItems[0]?.children : tabItems.length > 1 ? <Tabs items={tabItems} /> : null}
    </div>
  );
}
