...
后台按角色隔离：**管理员**可见本机菜单、全部任务/key、本机 agent；**普通登录用户**只能看自己的远程节点及其连接器上报的 agent、自己的任务空间（`web/<用户名>`，任务管理 / key / 概览统计）、通用对话，以及自己的微信绑定（账号槽=用户名，进程 `weixin:<username>`）。本机 agent 目录/启停/安装 API 返回 403。`canSeeNode` 对非管理员不包含内建 `local` 节点。微信 bot 用网关 token 读取登录用户任务列表；渠道 `ct_` 仍可读写自己的 `/v1` 会话。
