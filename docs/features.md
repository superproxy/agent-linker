...
- **Agent 是「按机器开通」的，不是全局配置**：
  - **本机节点 `local`**：agent 来自 `config.yaml` 的 `gateway.agents`，由网关进程在本机拉起；后台可启停、切模型、设为新任务默认（`GET /api/agents/by-node` 返回完整可编辑运行态）。**启停状态写入 `agents[].enabled`，重启不丢失**；切模型仍为运行时热更新。pi 的模型清单由本机 pi/ACP 自己维护，网关不写死火山等厂商模板；沙箱见 [`docs/pi.md`](pi.md)。
